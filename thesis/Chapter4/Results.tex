...
One of the key objectives of this thesis is to reduce the latency of hardware packet filtering on embedded systems, as such, measurement of the latency added due to the filtering is crucial.

\subsection{Theoretical analysis}
The design uses a 200 stage shift register to store the incoming packet where each stage is 2bits. At a clock frequency of 50Mhz, the added latency can be calculated to be $200 \times \frac{1}{50\times 10^6} = 4 \times 10^{-6} = 4\mu s$.

\subsection{Measured analysis}


To test the speed of the packet classifier, an Agilent MSO6054A MSO was used (4GSa/s sample rate). A pin was set to the output of the xor between the carrier sense line (crs\_dv) of the PHY and the crs\_dv after the packet filter. The added latency of the filter is then given by the time between the rising and falling edge of the output. The measured output can be seen to match the theoretical calculation of $4\mu s$ in figure \ref{fig:pf_added_latency}.
...
The two pulses in this graph are due to the PHY crs\_dv and delayed crs\_dv line having a phase shift and consequently a those two points, the value of the crs\_dv is different to the other. The distance between the two pulses indicates the size of the packet. 


\subsection{Improvements}
The result could be improved further to 0 added latency if the packet filter was integrated with the receive logic of the Ethernet MAC. This would involve storing the packet as it's coming in and then if the packet is deemed to be blocked, the FSM controlling the receive logic could be reset to ignore the packet. 
