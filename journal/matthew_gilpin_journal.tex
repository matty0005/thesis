...
\title{FPGA packet filter with Ethernet MAC with a RISC-V softcore processor}

\author{Matthew J. Gilpin \orcidlink{0009-0004-9748-4992}
        % <-this % stops a space
\thanks{Manuscript received ....; revised December, 2023}}

% The paper headers
\markboth{IEEE EMBEDDED SYSTEMS LETTERS,~Vol.~15, No.~3, November~2023}%
{FPGA packet filter with Ethernet MAC with a RISC-V softcore processor}
...
\begin{abstract}
Just like any other computer connected to the broader network, edge networks must also be safeguarded from malicious bad actors. Field programmable gate arrays (FPGAs) offer the flexibility of custom hardware that can be designed to incentivise low latency, high throughput yet efficient wire-speed firewalls. While current hardware firewalls exist in today's markets, they often come at a cost and high power usage rendering them unsuitable in edge networks. To address this, we designed a FPGA firewall that fulfils these criteria.
\end{abstract}

\begin{IEEEkeywords}
FPGA Packet filter, Ethernet MAC, RISC-V, Softcore processor
\end{IEEEkeywords}
	
\section{Introduction}
\IEEEPARstart{T}{his} \cite{IEEE802.3-2012} like any other computer connected to the broader network, edge networks must also be safeguarded from malicious bad actors. Field programmable gate arrays (FPGAs) offer the flexibility of custom hardware that can be designed to incentivise low latency, high throughput yet efficient wire-speed firewalls. While current hardware firewalls exist in today's markets, they often come at a cost and high power usage rendering them unsuitable in edge networks. To address this, we designed a FPGA firewall that fulfils these criteria.
